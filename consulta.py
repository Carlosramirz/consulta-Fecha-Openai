...
def get_rick_and_morty_character():
    """Get information about a random Rick and Morty character"""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return json.dumps({"name": name, "species": species, "status": status})
        else:
            return json.dumps({"error": f"Error {response.status_code}: {data['error']}"})
...
def ask_mia(request: UserRequest):
    ...
    tools = [
        ...
        "type": "function",
        "function": {
            "name": "get_rick_and_morty_character",
            "description": "Get information about a random Rick and Morty character",
            "parameters": {
                "type": "object",
                "properties": {},
            },
    ...
    if tool_calls:
        available_functions = {
            ...
            "get_rick_and_morty_character": get_rick_and_morty_character,
        }
        ...
        if conection:
            ...
            print("function_calling:", function_calling)
            cursor.execute(queryInsertUser, (uidChat, "user", request.user_message, 'none', 1))
            cursor.execute(queryInsertAssistant, (uidChat, "assistant", botResponse, function_calling, 1))
